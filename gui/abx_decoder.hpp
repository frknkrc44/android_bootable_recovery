...
class XMLElement {
 public:
	...
	XMLElement* subElementAt(int index) {
		if (index < 0 || index > subElements.size()) return NULL;
...
class AbxDecoder {
		...
		while (true) {
			...
			if (tType == TOKEN_START_DOCUMENT) {
				...
				if (dType == DATA_NULL) {
					...
					if (temp.size() > 0) value.assign(temp.begin(), temp.end());
 ...
 private:
	const short TOKEN_START_DOCUMENT = 0, TOKEN_END_DOCUMENT = 1, TOKEN_START_TAG = 2,
							TOKEN_END_TAG = 3, TOKEN_TEXT = 4, TOKEN_ATTRIBUTE = 15;

	const short DATA_NULL = 1 << 4, DATA_STRING = 2 << 4, DATA_STRING_INTERNED = 3 << 4,
							DATA_BYTES_HEX = 4 << 4, DATA_BYTES_BASE64 = 5 << 4, DATA_INT = 6 << 4,
							DATA_INT_HEX = 7 << 4, DATA_LONG = 8 << 4, DATA_LONG_HEX = 9 << 4,
							DATA_FLOAT = 10 << 4, DATA_DOUBLE = 11 << 4, DATA_BOOLEAN_TRUE = 12 << 4,
	...
	std::vector<char> base64_encode(std::vector<char> buf, int bufLen) {
		...
		while (bufLen--) {
			...
			if (i == 3) {
				...
				for (i = 0; (i < 4); i++) temp[i] = base64_chars[char_array_4[i]];
		...
		if (i) {
			...
			for (j = 0; (j < i + 1); j++) temp[j] = base64_chars[char_array_4[j]];
			ret.insert(ret.end(), temp, temp + 4);
			free(temp);

			temp = (char*)"=";
			while ((i++ < 3)) ret.insert(ret.end(), temp, temp + 1);
		}
	...
	void parseElement(XMLElement* element) {
		parsedElement += ("<");
		parsedElement.insert(parsedElement.end(), (*element).mTagName.begin(),
												 (*element).mTagName.end());
		...
		if ((*(*element).attributes).size() > 0) {
			for (const auto& it : (*(*element).attributes)) {
				...
				if (trimmedVal.size() > 0) parsedElement += (trimmedVal.data());
				parsedElement += ("\"");
			}
		}
